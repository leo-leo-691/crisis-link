...
async function generateDebriefReport(incident, timeline = [], tasks = [], messages = []) {
  ...
  return {
    executive_summary: `${String(incident?.type || 'incident').toUpperCase()} in ${incident?.zone || 'Unknown zone'} was handled through CrisisLink and closed in approximately ${resolutionMinutes} minutes.`,
    what_went_well: [
      ...
      completedTasks > 0 ? `${completedTasks} response tasks were completed during the incident.` : 'Core response tasks were available immediately after the incident was created.',
    ],
    ...
    root_cause_analysis: incident?.description
