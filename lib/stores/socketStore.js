...
function refreshOpenIncident(incidentId) {
  const store = useIncidentStore.getState();
  if (store.activeIncident?.incident?.id === incidentId) {
    store.fetchIncident(incidentId);
  }
}
  ...
  initSocket: (token) => {
    ...
    import('socket.io-client').then(({ io }) => {
      ...
      socket.on('incident:updated', (incident) => {
        ...
        if (incident?.id) refreshOpenIncident(incident.id);
      });

      socket.on('incident:message', (message) => {
        useIncidentStore.getState().addMessage(message);
        if (message?.incident_id) refreshOpenIncident(message.incident_id);
      });

      socket.on('incident:task', ({ incidentId, task }) => {
        if (incidentId) refreshOpenIncident(incidentId);
        import('./incidentStore').then(m => {
          const store = m.default.getState();
          if (store.activeIncident?.incident?.id === incidentId) {
            m.default.setState(state => {
              if (!state.activeIncident) return state;
              const tasks = state.activeIncident.tasks || [];
      ...
      socket.on('broadcast', (data) => {
        ...
        set(state => {
          // Prevent double-adding the exact same message if it arrives rapidly
          const isDuplicate = state.broadcasts.some(b => b.message === data.message && (Date.now() - new Date(b.timestamp).getTime() < 2000));
          if (isDuplicate) return state;
          
          return { broadcasts: [{ ...data, timestamp: new Date().toISOString() }, ...state.broadcasts].slice(0, 50) };
        });
      });
