...
export async function POST(request) {
  try {
    ...
    // Fetch context for AI in parallel
    const [staffResult, activeIncsResult] = await Promise.all([
      query("SELECT * FROM users WHERE role = 'staff' AND is_active = TRUE"),
      query("SELECT * FROM incidents WHERE status NOT IN ('resolved')")
    ]);

    const staff = staffResult.rows;
    const activeIncs = activeIncsResult.rows;

    // AI Execution Pipeline (Parallelized)
    ...
    const severity = triage?.severity || (type === 'fire' || type === 'medical' ? 'high' : 'medium');
    ...
    `, [
      ...
      triage ? JSON.stringify(triage) : null, provider, evacRoute, dispatchRec, is_drill,
      now, now,
    ]);
    ...
    return NextResponse.json({ 
      ...
      triage, 
      provider, 
      dispatch_recommendation: dispatchRec 
    }, { status: 201 });
